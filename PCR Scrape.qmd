...
# Extract the numbers from the bonus secodns column and convert rank to numeric.
...
oneday_race_results_fun <- function(race, year, session) {
                  ...
                  response <- scrape %>%
                                    ...
                                    html_table() %>%
                                    clean_names() %>%
                                    mutate(race_info = paste(race, year),
...
Now we have the results, need to have a look at it. Who had the most podiums for the the period? 
...
Since the start of the 2020 season Pog has won the most races/stages with 22 followed closely by Van Aert with 21 and Roglic at 18.
...
race_results_df %<>% 
                  mutate(race_level = case_when(race %in% c("giro-d-italia", 
                                                            "tour-de-france", 
                                                            "vuelta-a-espana") ~ "Grand Tour",
                                                race %in% c("milano-sanremo",
                                                            "ronde-van-vlaanderen",
                                                            "paris-roubaix",
                                                            "liege-bastogne-liege",
                                                            "il-lombardia") ~ "Momument",
                                                TRUE ~ "Other"))
...
one_day_race_wins <- race_results_df %>% 
                  ...
                  ggplot(aes(x = reorder(rider, total_wins), y = wins, fill = race_level)) +
                                    ...
                                    scale_y_continuous(breaks = c(0:20), limits = c(0,17)) +
                                    labs(x = "", title = "One-Day Races")
...
Perhaps surprisingly, Roglic had the most 2nd places since 2020. Recency bias coming in to play with regard to Van Aert for me on that one. 
